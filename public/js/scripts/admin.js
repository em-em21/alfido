...
(axios__WEBPACK_IMPORTED_MODULE_1___default().defaults.headers.common["X-Requested-With"]) = 'XMLHttpRequest'; // datatable defaults
...
$('.alert-success span, .alert-danger span').on('click', function () {
...
window.addEventListener('swal:confirm', function (event) {
  ...
  sweetAlert.fire({
    ...
    reverseButtons: true,
    customClass: {
      confirmButton: "confirmButton ".concat(sent.icon)
    }
  }).then(function (result) {
    if (result.isConfirmed) {
      // console.log('confirmed indeeeeeeeeeeeeeeeeeeeed')
      if (sent.model === 'user:delete') window.livewire.emit('user:delete');
      if (sent.model === 'user:ban') window.livewire.emit('user:ban');
      if (sent.model === 'user:revive') window.livewire.emit('user:revive');
      if (sent.model === 'deal:delete') window.livewire.emit('deal:delete', sent.id);
      if (sent.model === 'trans:delete') window.livewire.emit('trans:delete', sent.id);
      if (sent.model === 'manager:delete') window.livewire.emit('manager:delete', sent.id);
...
window.addEventListener('admin:user-operations', function (event) {
  (0,_components_user_Alerts__WEBPACK_IMPORTED_MODULE_2__.default)(event.detail.message, event.detail.status);
}); /////////////////////// SIMPLE TABLE
...
$(document).ready(function () {
  $('.simple-table').each(function (i) {
    ...
    $(this).find('th').each(function (i) {
      $('#' + id + ' td:nth-child(' + (i + 1) + ')').prepend('<span class="simple-table-thead">' + $(this).text() + ':</span> ');
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  if (document.querySelector('.chat')) {
    var fetchUsers = function fetchUsers() {
      axios__WEBPACK_IMPORTED_MODULE_1___default().get('/admin/chat/fetch-users').then(function (r) {
        usersCont.innerHTML = r.data;
        var users = usersCont.querySelectorAll('.user');
        iterateUsers(users);
      })["catch"](function (err) {
    ...
    var fetchMessages = function fetchMessages() {
      axios__WEBPACK_IMPORTED_MODULE_1___default().get('/admin/chat/fetch-messages/' + receiverID).then(function (r) {
    ...
    var iterateUsers = function iterateUsers(TARGETS) {
      for (var i = 0; i < TARGETS.length; i++) {
        var self = TARGETS[i];
        self.addEventListener('click', function (e) {
          document.querySelectorAll('.user').forEach(function (i) {
            return i.classList.remove('active');
          }); // geniusly, right?

          var thisUser = e.target.closest('.chat__users--item');
     ...
     * Responsive chat
     */
    var showHideChats = function showHideChats() {
      var chatUsersContainer = document.querySelector('.chat__users'),
      ...
      if (window.innerWidth < 700) {
        ...
        chatUsers.forEach(function (el, i) {
    ...
    var pusher = new Pusher('d2059ddf4f6dd4d07d04', {
      cluster: 'ap2',
      forceTLS: true
    });
    ...
    channel.bind('chat-message-sent', function (data) {
      ...
      var messageTo = document.getElementById(msg.to);
      var messageFrom = document.getElementById(msg.from);
    ...
    submitMessage.addEventListener('click', function (e) {
      ...
      if (message != '' && receiverID != '') {
        axios__WEBPACK_IMPORTED_MODULE_1___default().post('/admin/chat/send-message', {
    ...
    messageInput.addEventListener('focusin', function (e) {
