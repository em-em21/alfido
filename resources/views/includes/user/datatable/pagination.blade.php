<div>
  @if ($paginator->hasPages())
    <nav role="navigation" aria-label="Pagination Navigation" class="flex items-center justify-between">
      {{-- small screen --}}
      <div class="flex justify-between flex-1 sm:hidden mb-2 space-x-2">
        {{-- Previous --}}
        <span>
          @if ($paginator->onFirstPage())
            <span
              class="relative h-full inline-flex items-center px-4 py-2 text-sm font-medium bg-gray-800 text-gray-400 bg-white border border-gray-600 cursor-default leading-6 rounded-md opacity-50">
              {!! __('pagination.previous') !!}
            </span>
          @else
            <a href="{{ url()->current() . '?page=' . max($this->page - 1, 1) }}" rel="prev"
                  class="relative h-full inline-flex items-center px-3 py-2 text-sm font-medium bg-indigo-500 text-gray-100 border border-gray-700 rounded-md leading-6 hover:text-gray-300 hover:border-gray-500 focus:z-10 focus:outline-none focus:border-gray-200 focus:text-gray-200"
        ...
        <span>
          @if ($paginator->hasMorePages())
                <a href="{{ url()->current() . '?page=' . ($this->page + 1) }}" class="relative h-full inline-flex items-center px-3 py-2 text-sm font-medium bg-indigo-500 text-gray-100 border border-gray-700 rounded-md leading-6 hover:text-gray-300 hover:border-gray-500 focus:z-10 focus:outline-none focus:border-gray-200 focus:text-gray-200">
                    {!! __('pagination.next') !!}
                </a>
          @else
            <span
              class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-400 bg-gray-800 border border-gray-600 cursor-default leading-6 rounded-md opacity-50">
              {!! __('pagination.next') !!}
            </span>
          @endif
        </span>
      ...
      {{-- desktop --}}
      <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between flex-col-reverse mb-2">
        <div>
                  ...
                  <span wire:key="paginator-page{{ $page }}">
                    @if ($page == $paginator->currentPage())
                      <span aria-current="page">
                        <span
                          class="relative h-full inline-flex items-center px-4 py-2 -ml-px z-10 font-medium border border-gray-500 bg-indigo-500 text-gray-100 cursor-default leading-6">{{ $page }}</span>
                      </span>
                    @else
