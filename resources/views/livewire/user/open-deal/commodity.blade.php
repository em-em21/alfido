<div class="card__body h-full market_commo">
    <form 
		x-data="{
			dest: 1,
			btnActive: false,
	...
	>
			...
			<div class="select w-full block relative">
				...
				>
					@foreach($stocks as $stock)
						<option class="text-gray-500 nunito text-lg" value="{{ $stock->alias }}">{{ $stock->name }} | {{ $stock->alias }}</option>
					@endforeach
				</select>
			</div>
		</div>
		...
		<div class="form-group flex flex-col items-start">
			<input 
				:class="{'border-green-800': dest, 'focus:border-green-800': dest, 'border-red-800': !dest, 'focus:border-red-800': !dest}"
				class="input__buy-sell w-full nunito bg-transparent text-gray-400 text-lg rounded shadow-md transition ease-out duration-100 px-3 py-2 focus:shadow-none focus:text-gray-50 focus:ring-0"
		...
		{{-- Submit --}}
		<button 
			class="px-3 py-2 rounded-md shadow-md transition-all ease-in duration-150 border border-transparent btn-bs checker {{ auth()->user()->is_baned ? 'baned' : '' }} {{ $errors->any() ? 'offmode' : '' }}"
			...
			type="submit"
		>
			<span class="text-gray-50 text-lg" x-text="dest ? 'Купить' : 'Продать'" ></span>
